...
const chartConfig = {
  usd_close: { label: 'USD 가격', color: 'var(--chart-1)' },
  krw_close: { label: 'KRW 가격', color: 'var(--chart-2)' },
  exchange_rate: { label: '환율', color: 'var(--chart-3)' },
  usd: { label: 'USD 수익률', color: 'var(--chart-1)' },
  krw: { label: 'KRW 수익률', color: 'var(--chart-2)' },
  usdDrawdown: { label: 'USD 낙폭', color: 'var(--chart-1)' },
  krwDrawdown: { label: 'KRW 낙폭', color: 'var(--chart-2)' },
        ...
        {activeTab === 'usd' && (
          <ChartContainer config={chartConfig} className="h-[400px] w-full aspect-auto">
            <ComposedChart data={chartData}>
              ...
              <Area type="monotone" dataKey="usd_close" stroke="var(--chart-1)" fill="url(#usdGradient)" strokeWidth={2} />
            </ComposedChart>
          </ChartContainer>
        )}
        ...
        {activeTab === 'compare' && (
          <ChartContainer config={chartConfig} className="h-[400px] w-full aspect-auto">
            <LineChart data={chartData}>
              ...
              />
              <Legend />
              <Line yAxisId="usd" type="monotone" dataKey="usd_close" stroke="var(--chart-1)" strokeWidth={2} dot={false} name="USD 가격" />
              <Line yAxisId="krw" type="monotone" dataKey="krw_close" stroke="var(--chart-2)" strokeWidth={2} dot={false} name="KRW 가격" />
            </LineChart>
          </ChartContainer>
        )}
          ...
          <>
            ...
            <ChartContainer config={chartConfig} className="h-[400px] w-full aspect-auto">
              <LineChart data={analysis.normalizedReturns}>
                ...
                />
                <Legend />
                <Line type="monotone" dataKey="usd" stroke="var(--chart-1)" strokeWidth={2} dot={false} name="USD 수익률" />
                <Line type="monotone" dataKey="krw" stroke="var(--chart-2)" strokeWidth={2} dot={false} name="KRW 수익률" />
              </LineChart>
            </ChartContainer>
          </>
        )}

        {activeTab === 'drawdown' && analysis && (
          <>
            <div className="flex gap-4 mb-3 text-sm">
              <span className="text-muted-foreground">
            ...
            <ChartContainer config={chartConfig} className="h-[400px] w-full aspect-auto">
              <ComposedChart data={analysis.drawdown.usdSeries.map((item, i) => ({
                ...
                <Area type="monotone" dataKey="usdDrawdown" stroke="var(--chart-1)" fill="url(#ddUsdGrad)" strokeWidth={2} name="USD 낙폭" />
                <Line type="monotone" dataKey="krwDrawdown" stroke="var(--chart-2)" strokeWidth={1.5} dot={false} strokeDasharray="4 2" name="KRW 낙폭" />
              </ComposedChart>
            </ChartContainer>
          </>
        )}
        ...
        {activeTab === 'exchange' && (
          <ChartContainer config={chartConfig} className="h-[400px] w-full aspect-auto">
            <LineChart data={chartData}>
              ...
              />
              <Line type="monotone" dataKey="exchange_rate" stroke="var(--chart-3)" strokeWidth={2} dot={false} name="환율" />
            </LineChart>
          </ChartContainer>
        )}
      </CardContent>
